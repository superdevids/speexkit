...
export function histogram(
  data: number[],
  o?: { bins?: number | 'sturges' | 'fd' | 'sqrt'; range?: [number, number] },
): { counts: number[]; binEdges: number[]; binWidth: number } {
  ...
  const nb =
    typeof o?.bins === 'number'
      ? o.bins
      : o?.bins === 'fd'
        ? freedmanDiaconisBins(d)
        : o?.bins === 'sqrt'
          ? Math.ceil(Math.sqrt(d.length))
          : sturgesBins(d)
  const bw = (mx - mn) / nb
  const be: number[] = []
  for (let i = 0; i <= nb; i++) be.push(mn + i * bw)
  const c: number[] = new Array(nb).fill(0)
  for (const v of d) {
    if (v === mx) {
      c[nb - 1]!++
    } else {
      const idx = Math.floor((v - mn) / bw)
...
export function kde(data: number[], bandwidth?: number, gridPoints = 512): { x: number[]; density: number[] } {
  ...
  for (const v of data) std += (v - m) ** 2
  std = Math.sqrt(std / (n - 1))
  const sigma = Math.min(std, iqrVal / 1.34)
  let h = bandwidth ?? sigma * (4 / (3 * n)) ** (1 / 5)
  if (h <= 0) h = 0.1
  const pad = h * 3,
    gMin = s[0]! - pad,
    gMax = s[n - 1]! + pad,
    step = (gMax - gMin) / (gridPoints - 1)
  const x: number[] = [],
    density: number[] = []
...
export function colorMap(name: string, n = 256): string[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const lo = Math.floor(idx),
      hi = Math.ceil(idx)
    if (lo === hi) {
      result.push(stops[lo]!)
    } else {
      const t = idx - lo
      const interp = (c1: string, c2: string, o: number) =>
        Math.round(parseInt(c1.slice(o, 2), 16) * (1 - t) + parseInt(c2.slice(o, 2), 16) * t)
      result.push(
        `#${[1, 3, 5]
          .map((o) =>
            Math.min(255, Math.max(0, interp(stops[lo]!, stops[hi]!, o)))
              .toString(16)
              .padStart(2, '0'),
          )
          .join('')}`,
