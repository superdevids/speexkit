...
function buildCategory(name: string, base: string, toBase: Record<string, number>, units?: string[]): CategoryDef {
 ...
 */
export const UNIT_CATEGORIES: Record<string, { name: string; units: string[]; base: string }> = Object.fromEntries(
...
export function convert(value: number, from: string, to: string): number {
  ...
  if (UNIT_TO_CATEGORY[to] !== cat) {
    throw new Error(`Cannot convert "${from}" (${cat}) to "${to}" (${UNIT_TO_CATEGORY[to] ?? 'unknown'})`)
  }
 ...
 */
export function convertWithCategory(value: number, from: string, to: string): { value: number; category: string } {
  const cat = UNIT_TO_CATEGORY[from]
  if (cat === undefined) {
    throw new Error(`Unknown unit: "${from}"`)
  }
